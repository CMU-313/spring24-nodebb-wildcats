...
Flags.init = async function () {
  ...
  try {
    ...
    } = await plugins.hooks.fire('filter:flags.getFilters', hookData));
    ({
      filters: Flags._filters,
      states: Flags._states
...
Flags.create = async function (type, id, uid, reason, timestamp, forceFlag = false) {
  ...
  if (targetFlagged) {
    ...
    await Promise.all([Flags.addReport(flagId, type, id, uid, reason, timestamp), Flags.update(flagId, uid, {
      state: 'open'
    })]);
  ...
  batched.push(db.setObject(`flag:${flagId}`, {
    ...
    targetUid: targetUid,
    datetime: timestamp
  }), Flags.addReport(flagId, type, id, uid, reason, timestamp), db.sortedSetAdd('flags:datetime', timestamp, flagId),
  // by time, the default
  db.sortedSetAdd(`flags:byType:${type}`, timestamp, flagId),
  // by flag type
  db.sortedSetIncrBy('flags:byTarget', 1, [type, id].join(':')),
  ...
  if (type === 'post') {
    batched.push(db.sortedSetAdd(`flags:byPid:${id}`, timestamp, flagId),
    // by target pid
